...
// Output thresholds with hysteresis: press above, release below.
constexpr double PRESS_THRESHOLD = 0.55;
constexpr double RELEASE_THRESHOLD = 0.45;
// Fitness (in percent) docked per button press. Small enough that real
// progress always dominates, large enough that at equal distance the
// genome with fewer jumps wins.
constexpr double JUMP_PENALTY = 0.02;
// Frontier takeover window, in decision steps (~240 per second of game
// time - one per engine substep). A child replays its parent's tape until
// `lead` steps before the parent's death, then its network takes over: at
// least LEAD_MIN so the network has time to act before the obstacle that
// killed the parent, randomized up to LEAD_MAX because the fix for a failed
// jump is sometimes an earlier setup, not a different last-moment input.
...
bool NEATManager::beginTraining(TrainingConfig config) {
    ...
    // The fast-forward happens in the CCScheduler::update hook, gated on
    // phase == Training.
...
void NEATManager::stop(char const* reason) {
    ...
    // A cancelled or quit-out training run still keeps its best result in
    // the playback library - the overnight scenario the library exists for
    // is exactly the one where nobody was around when it ended. (Showcase
    // runs were already saved when their training finished.)
...
bool NEATManager::beginAttempt() {
    ...
    if (m_cursor >= m_population->size()) return false; // shouldn't happen
    Genome const& g = m_population->genome(m_cursor);
    m_currentNet = std::make_unique<Network>(g);

    // Frontier exploration: replay the inherited tape verbatim until just
    // before the parent's death step, then hand control to the (mutated)
    // network. A genome that has never run (fresh/first generation) has
    // reachStep 0, so its network controls from the start.
...
bool NEATManager::shouldHoldCurrent(
    ...
    if (step < m_takeoverStep) {
        // Replayed prefix: the parent's proven button history, open-loop.
        hold = tapeStateAt(step);
    } else {
        double const out = m_currentNet->eval(inputs);
        hold = out > (holding ? RELEASE_THRESHOLD : PRESS_THRESHOLD);
    }
    // Record the attempt's actual history (replayed prefix + the network's
    // own decisions): it becomes this genome's tape, inherited by children.
...
void NEATManager::endAttempt(double percent, bool died, int jumps) {
    ...
    // Death is reported once per engine call; hazard overlap (or a reset on
    // the heels of a death) would otherwise score the *next* genome too.
    if (!m_attemptActive) return;
    m_attemptActive = false;
    m_currentNet.reset();

    // Persist the attempt's actual input history onto the genome before
    // anything copies it: children inherit it through epoch(), and the
    // best-genome snapshot below carries it into the showcase.
    m_population->setTape(m_cursor, std::move(m_recordToggles), m_step);
    m_recordToggles.clear(); // moved-from; make it definitively empty

    // Selection sees the jump-efficiency-adjusted score; best tracking and
    // the HUD below stay on the raw percent.
    ...
    if (percent > m_bestFitness) {
        ...
        m_sinceImproved = -1; // bumped back to 0 at the generation boundary
    }
    ...
    // Selection is driven by raw distance (minus the small jump penalty).
    // Diversity is the job of speciation inside epoch(), which shares fitness
    // across network *topologies* - not across death locations. An earlier
    // version niched by outcome here (fitness / sqrt(genomes dying nearby)),
    // but on a level where almost everything dies at the first obstacle that
    // cluster IS the population, so it divided the front-runners by ~sqrt(pop)
    // and let stragglers that died alone out-compete them. That inverted the
    // gradient and parked the whole run at the first jump - the opposite of
    // what niching was meant to do.

    // Stuck for a few generations: raise mutation pressure and inject fresh
    // random genomes until something improves again.
    ...
    // Carry the all-time best into the next generation unchanged, so a good
    // line is never lost to mutation/crossover (it was previously kept only
    // for the showcase, never re-bred).
...
void NEATManager::finishTraining(bool solved, char const* reason) {
    // Showcase runs at 1x and is meant to be watched, so drop back to the
    // player's normal frame rate as training ends.
    restoreFps();
    // Persist the run into the level's playback library before anything
    // else: the watch-now prompt below is skippable, the file is the
    // durable record.
    savePlayback();
    ...
    // Don't launch the one-shot showcase unprompted: training regularly
    // finishes unattended, and an auto-played showcase would be over before
    // anyone saw it. The PlayLayer hook pauses the game and asks instead;
    // the run is already in the playback library either way.
    m_phase = Phase::Idle;
...
void NEATManager::savePlayback() {
    ...
    std::tm tm{};
    localtime_s(&tm, &t); // Windows-only mod (see mod.json)
    char buf[32] = {};
...
bool NEATManager::armShowcase() {
    // Only meaningful right after a finished training session: the trained
    // best genome (network + tape) is still held.
...
bool NEATManager::beginPlayback(Playback const& playback) {
    ...
    // Tape-only stand-in for the showcase genome: nodeCount 0 means
    // onShowcaseStart builds no network and the run is pure replay.
    m_bestGenome = Genome{};
    m_bestGenome.tapeToggles = playback.toggles;
    m_bestGenome.reachStep = playback.reachStep;
    m_bestFitness = playback.percent; // showcase HUD
    m_showcaseActive = false;
    m_phase = Phase::Showcase;
...
void NEATManager::onShowcaseStart() {
    ...
    // Saved playbacks are tape-only (nodeCount 0): no network fallback, the
    // recorded button history is the whole run.
    m_showcaseNet = m_bestGenome.nodeCount > 0
        ? std::make_unique<Network>(m_bestGenome)
        : nullptr;
    // The showcase replays the best genome's full recorded tape - the run
    // being showcased IS that tape (its network only ever controlled the
    // final stretch). Past the tape's end (only reachable if the genome
    // beat the level, or under engine nondeterminism) the network takes
    // over as a fallback.
...
bool NEATManager::shouldHold(std::vector<double> const& inputs, bool holding) {
    ...
    // Tape-only playback: past the tape's end (the recorded run is over),
    // just keep the button released until death/level end.
...
std::string NEATManager::progressText() const {
    ...
    // Pace-based estimate against the generation cap, additionally capped
    // by the remaining time budget. Early stopping (stagnation / solve)
    // makes this an upper bound.
    std::string eta = "--:--";
